...
function ItemDAO(database) {
    ...
    this.getCategories = function(callback) {
        ...
        */
        ...
        database.collection('item', function (err, col) {
        assert.equal(null,err);
            ...
            cursor.toArray(function (err,docs) {
                ...
                //Aşağıdaki kısmı da toArray fonksiyonuna dahil ettik, toArray async bir fonk olduğundan shift fonksiyonunu dışında yazarsak dizi halen boş olduğundan exception veriyor.
                //dizideki toplam sayı bulunuyor
                var sonuc = 0 ;
                categories.forEach(function (elem) {  
                    sonuc = sonuc + elem.num;
                });
                var category = {
                    _id: "All",
                    num: sonuc
                ...
                //sonunda callback'i db'den çektiği categories dizisiyle çağırıyor
                callback(categories);  
            });         
        });
    ...
    this.getItems = function(category, page, itemsPerPage, callback) {
        ...
        //page burada 0 dan başlamış gelirken, yoksa -1 yapmak gerekirdi.
    ...
    this.searchItems = function(query, page, itemsPerPage, callback) {
        ...
        // var komut = { $text: {$search: query}};

        // var cursor = database.collection('item').find(komut);
        // cursor.limit(itemsPerPage);
        // //page burada 0 dan başlamış gelirken, yoksa -1 yapmak gerekirdi.
        // cursor.skip(itemsPerPage * page);
        // cursor.sort([['_id',1]]);
    ...
    this.getNumSearchItems = function(query, callback) {
        ...
        */
        var komut = { $text: {$search: query}};
                ...
                cursor.count(function(err, sayac){
                    assert.equal(null,err);
                    callback(sayac);
                });   
    ...
    this.addReview = function(itemId, comment, name, stars, callback) {
        ...
        database.collection('item').updateOne(filter,update,options,function (err,result) {
            assert.equal(null,err);
            callback({_id:itemId,reviews:[reviewDoc]});
        });
