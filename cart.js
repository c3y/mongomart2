...
function CartDAO(database) {
    ...
    //bu function'dan oluşacak objelerin db diye bir property'si olacak.  
    this.db = database;
    ...
    this.getCart = function(userId, callback) {
        ...
        *
        */
        var komut = {"userId":userId};
        var cursor = database.collection('cart').find(komut);
    ...
    this.itemInCart = function(userId, itemId, callback) {
         ...
         */
         ...
         var project = {"items.$":1};
         var cursor = database.collection('cart').find(query,project).limit(1);
         cursor.next(function (err,res) {
             assert.equal(null, err);           
             callback(res == null ? null : res.items[0]);
         });
